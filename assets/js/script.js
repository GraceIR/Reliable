...
(function ($) {
 ...
 // =========Leaflet map=========>>>>>
if ($('#map').length > 0) {
  // Center on Lagos, Nigeria [latitude, longitude]
  var map = L.map('map').setView([6.5244, 3.3792], 10);
  var locationsArray = [];

  function clickZoom(e) {
    map.setView(e.target.getLatLng(), 12); // Zoom to level 12 on click
  }

  // Lagos locations data - you can add more locations here
  var lagosLocations = [
    {
      title: "Lagos Mainland",
      subtitle: "Central Business District",
      address: "Lagos Mainland, Lagos",
      markerPoint: [6.4551, 3.3841]
    },
    {
      title: "Victoria Island",
      subtitle: "Financial Hub",
      address: "Victoria Island, Lagos",
      markerPoint: [6.4281, 3.4218]
    },
    {
      title: "Ikeja",
      subtitle: "State Capital",
      address: "Ikeja, Lagos",
      markerPoint: [6.6018, 3.3515]
    },
    {
      title: "Lekki",
      subtitle: "Residential Area",
      address: "Lekki, Lagos",
      markerPoint: [6.4627, 3.5362]
    },
    {
      title: "Apapa",
      subtitle: "Port Area",
      address: "Apapa, Lagos",
      markerPoint: [6.4489, 3.3599]
    }
  ];

  $.each(lagosLocations, function(index, location) {
    // Create Marker
    var marker = L.marker(location.markerPoint, {
      title: location.title,
      className: "marker-lagos"  // Changed class name
    }).addTo(map);

    // Bind Popup
    marker.bindPopup(`<div class="card logistics-map-card"><div class="card-body">
                        <h5 class="card-title location-title">${location.title}</h5>
                        <p class="mb-0 fw-semibold location-subtitle">${location.subtitle}</p>
                        <p class="mb-0 small location-details">${location.address}</p>
  ...
  // Using OpenStreetMap tiles for Nigeria
  ...
  document.addEventListener("DOMContentLoaded", function() {
    // Decode URL so special characters like apostrophes match
